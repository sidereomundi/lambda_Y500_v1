...
Function_1D* Get_P_M_lambda (Parameters_struct* params,
	...
	//	for(i = 0; i<MassFunc->Nx; i++)printf("%f %f %lf\n",redshift,MassFunc->x[i],1.e12*dNdM_redshift[i]);
...
/*   	for (i=0;i<MassFunc->Nx;i++)printf("%d %f %f \n",0,MassFunc->x[i],dNdM_redshift[i]*1.e12);   */
/*   	for (i=0;i<n;i++)printf("%d %f %f \n",1,result->x[i],dNdM[i]*1.e12);   */
	...
	regulzeta_ok = Regularize_1D(result, 0., REGUL_LOG);
	
	//	for(i = 0; i < n; i++)printf("%lf %lf %lf %lf \n",result->x[i],P_lambda0_lambdaobs->y[i],dNdM[i]*1.e12,result->y[i]*1.e12);
